...
const loadProjects = async () => {
  ...
  try {
    ...
    projects.value = mergeProjects(githubRepos, fallbackProjects)
  } catch (err) {
...
<template>
  <section id="projects" class="py-20 bg-white dark:bg-dark-surface transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- Error Message -->
      <div v-else-if="error" class="bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-400 dark:border-yellow-700 text-yellow-800 dark:text-yellow-300 px-4 py-3 rounded-lg mb-8">
